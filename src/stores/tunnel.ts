...
export const useTunnelStore = defineStore("tunnel", () => {
  ...
  const stoppedTunnels = computed(() =>
    tunnels.value.filter(t =>
      t && t.id && t.status === "stopped"
    )
  );

  const errorTunnels = computed(() =>
    tunnels.value.filter(t =>
      t && t.id && t.status === "error"
    )
  );
  ...
  const loadTunnels = async () => {
    ...
    try {
      ...
      // Validate and filter out any invalid tunnel data
      tunnels.value = (loadedTunnels || []).filter(tunnel =>
        tunnel &&
        tunnel.id &&
  ...
  const createTunnel = async (request: CreateSSHTunnelRequest): Promise<SSHTunnel> => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to create tunnel';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const updateTunnel = async (id: string, request: UpdateSSHTunnelRequest): Promise<SSHTunnel> => {
    loading.value = true;
    error.value = null;
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to update tunnel';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const deleteTunnel = async (id: string): Promise<void> => {
    ...
    try {
      ...
      error.value = err instanceof Error ? err.message : 'Failed to delete tunnel';
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const startTunnel = async (id: string): Promise<void> => {
    ...
    try {
      ...
      if (tunnel) {
        tunnel.status = "error";
        tunnel.errorMessage = err instanceof Error ? err.message : 'Failed to start tunnel';
      }
      throw err;
    }
  };
  ...
  const stopTunnel = async (id: string): Promise<void> => {
    ...
    try {
      ...
      if (tunnel) {
        tunnel.status = "error";
        tunnel.errorMessage = err instanceof Error ? err.message : 'Failed to stop tunnel';
      }
      throw err;
    }
  };
  ...
  const refreshAllTunnelStatus = async (): Promise<void> => {
    try {
      ...
      // Validate and filter out any invalid tunnel data
      tunnels.value = (updatedTunnels || []).filter(tunnel =>
        tunnel &&
        tunnel.id &&
        typeof tunnel.id === 'string'
      );
