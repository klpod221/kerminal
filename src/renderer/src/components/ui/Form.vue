...
const isValid = computed(() => {
  // Valid if no child has an error after validation
...
const getFieldValue = (id: string): unknown => {
  ...
  if (field && field.value && typeof field.value === 'object' && 'value' in field.value) {
...
const getAllFieldValues = (): Record<string, unknown> => {
  ...
  fields.forEach((field, id) => {
    if (field.value && typeof field.value === 'object' && 'value' in field.value) {
