...
use crate::{
    ...
    models::{
        saved_command::{SavedCommand, SavedCommandGroup},
...
impl Database for SQLiteProvider {
    ...
    async fn create_tables(&self) -> DatabaseResult<()> {
        ...
        // Create saved command groups table
        sqlx::query(
            r#"
...
impl SQLiteProvider {
    ...
    pub async fn find_saved_commands_by_group_id(&self, group_id: Option<&str>) -> DatabaseResult<Vec<SavedCommand>> {
        let pool = self.get_pool()?;
        let pool = pool.read().await;
    ...
    pub async fn find_saved_command_group_by_id(&self, id: &str) -> DatabaseResult<Option<SavedCommandGroup>> {
        let pool = self.get_pool()?;
        let pool = pool.read().await;
