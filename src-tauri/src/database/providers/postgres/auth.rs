...
pub async fn save_device(provider: &PostgreSQLProvider, device: &Device) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn get_current_device(provider: &PostgreSQLProvider) -> DatabaseResult<Option<Device>> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn get_all_devices(provider: &PostgreSQLProvider) -> DatabaseResult<Vec<Device>> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn delete_device(provider: &PostgreSQLProvider, device_id: &str) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
