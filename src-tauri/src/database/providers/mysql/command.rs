...
pub async fn delete_saved_command(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn increment_command_usage(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn toggle_command_favorite(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn delete_saved_command_group(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
