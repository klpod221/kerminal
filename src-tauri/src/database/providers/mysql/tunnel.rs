...
pub async fn save_ssh_tunnel(provider: &MySQLProvider, model: &SSHTunnel) -> DatabaseResult<()> {
...
pub async fn find_all_ssh_tunnels(provider: &MySQLProvider) -> DatabaseResult<Vec<SSHTunnel>> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn delete_ssh_tunnel(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
