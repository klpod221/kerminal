...
pub async fn save_ssh_profile(provider: &MySQLProvider, model: &SSHProfile) -> DatabaseResult<()> {
...
pub async fn find_all_ssh_profiles(provider: &MySQLProvider) -> DatabaseResult<Vec<SSHProfile>> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn delete_ssh_profile(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn save_ssh_group(provider: &MySQLProvider, model: &SSHGroup) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn find_all_ssh_groups(provider: &MySQLProvider) -> DatabaseResult<Vec<SSHGroup>> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn update_ssh_group(provider: &MySQLProvider, model: &SSHGroup) -> DatabaseResult<()> {
    save_ssh_group(provider, model).await
}

pub async fn delete_ssh_group(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn save_ssh_key(provider: &MySQLProvider, model: &SSHKey) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn find_all_ssh_keys(provider: &MySQLProvider) -> DatabaseResult<Vec<SSHKey>> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
...
pub async fn update_ssh_key(provider: &MySQLProvider, model: &SSHKey) -> DatabaseResult<()> {
    save_ssh_key(provider, model).await
}

pub async fn delete_ssh_key(provider: &MySQLProvider, id: &str) -> DatabaseResult<()> {
    let pool_arc = provider.get_pool()?;
    let pool = pool_arc.read().await;
...
pub async fn count_profiles_using_key(provider: &MySQLProvider, key_id: &str) -> DatabaseResult<u32> {
    let pool = provider.get_pool()?;
    let pool = pool.read().await;
